...
public class ThreeDNeighbourhoodCalculator extends AbstractNeighbourhoodCalculator implements StateCodeCalculator {
	private ObjectCodeImpl previousObjectCode = new ObjectCodeImpl();
	private ObjectCodeImpl nextObjectCode = new ObjectCodeImpl();

	public ThreeDNeighbourhoodCalculator(Model model, List<? extends Interpretation<?>> interpretations, IntSet individuals) {
	...
	public StateCoderResult calculateCodes() {
		...
		do {
			...
			nextObjectCode.clear();
			rounds++;
		} while (rounds <= 3 && rounds <= previousObjectCode.getEffectiveSize());
